...
class WelcomeViewController: UIViewController, WelcomeDisplayLogic
{
  ...
  func login()
  {
    let myTracker = ADEumInstrumentation.beginCall(self, selector: #function)
    let userID = userIDTextField.text
    let password = passwordTextField.text
    ADEumInstrumentation.reportMetric(withName: "login called", value: 1)
    ADEumInstrumentation.setUserData("UserID", value: userID ?? "none")
    if userID == "crash"{
        fatalError()
    }
    ADEumInstrumentation.endCall(myTracker)
